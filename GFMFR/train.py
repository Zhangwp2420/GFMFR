...
import os
import argparse
from models.GFMFR import GFMFREngine
...
if __name__ == "__main__":
    ...
    logname = os.path.join(config['logging_path'], current_time + '-' + config['dataset'] + '-' + config['eval_setting']+'.txt')
    initLogging(logname)
    ...
    for round_i in range(config['num_round']):
        ...
        val_hit_ratio, val_ndcg = enginer.fed_evaluate(validate_data)
        logging.info(
        ...
        hit_ratio, ndcg = enginer.fed_evaluate(test_data)
